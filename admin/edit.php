...
ini_set('display_errors', '0'); // Hide errors from being displayed to the user
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_product'])) {
    ...
    if ($stmt->execute()) {
        ...
        // Redirect to view.php (you can modify the URL as needed)
        header("Location: view.php?id=" . $id); // Redirect to view.php with the current product id
        exit;
    } else {
...
<?php include 'header.php';?>
    ...
    <h2>Product Details (Image Links)</h2>
<form method="POST">
    <input type="url" name="value" placeholder="Enter Image URL" required>
    <button type="submit" name="action" value="add_link">Add Image</button>
</form>
<div class="slider-images">
    <?php 
    $product_details = json_decode($product['product_details'], true) ?? [];
    foreach ($product_details as $image_link): ?>
        <div>
            <img src="<?= htmlspecialchars($image_link) ?>" alt="Product Detail Image" style="max-width: 100px; max-height: 100px; object-fit: cover; border: 1px solid #ccc;">
            <form method="POST">
                <input type="hidden" name="value" value="<?= htmlspecialchars($image_link) ?>">
...
<?php if ($product): ?>
<form method="POST">
    <button class="delete-button" name="delete_product" onclick="return confirm('Are you sure you want to delete this product?');">
        Delete Product
    </button>
</form>
<?php endif; ?>
